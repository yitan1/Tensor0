"""Public materialization layouts, conversions and validation."""
...
@pytest.mark.parametrize("source_dtype,result_dtype", [
    pytest.param(source, result, id=f"{result}-{source}")
    for result in CONVERSION_DTYPES for source in CONVERSION_DTYPES
    if source == result or (source, result) in MATERIALIZE_MIXED_PAIRS
    or (result, source) in MATERIALIZE_MIXED_PAIRS
])
