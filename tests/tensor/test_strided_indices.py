...
def test_gather_and_scatter_strided_use_same_values_after_cache_clear():
    _blocks._clear_strided_indices_cache_for_tests()
    data = jnp.arange(12.0)

    gathered = _blocks.gather_strided(data, (2, 3), (1, 4), 2)
    scattered = _blocks.scatter_add_strided(
        jnp.zeros_like(data),
        (2, 3),
        (1, 4),
        2,
    ...
    assert gathered.tolist() == [[2.0, 6.0, 10.0], [3.0, 7.0, 11.0]]
...
def test_jitted_gather_matches_expected_result():
    _blocks._clear_strided_indices_cache_for_tests()
    data = jnp.arange(12.0)

    @jax.jit
    def gather_once(value):
        return _blocks.gather_strided(value, (2, 3), (1, 4), 2)

    result = gather_once(data)
...
def test_contiguous_gather_matches_flat_slice_without_populating_cache():
    _blocks._clear_strided_indices_cache_for_tests()
    data = jnp.arange(12.0)

    gathered = _blocks.gather_strided(data, (2, 3), (3, 1), 4)

    assert gathered.tolist() == [[4.0, 5.0, 6.0], [7.0, 8.0, 9.0]]
    assert len(_blocks._STRIDED_INDICES_CACHE) == 0


def test_contiguous_scatter_add_matches_flat_slice_without_populating_cache():
    _blocks._clear_strided_indices_cache_for_tests()
    storage = jnp.zeros((12,), dtype=jnp.float32)
    value = jnp.array([[4.0, 5.0, 6.0], [7.0, 8.0, 9.0]], dtype=jnp.float32)

    result = _blocks.scatter_add_strided(storage, (2, 3), (3, 1), 4, value)

    assert result.tolist() == [0.0, 0.0, 0.0, 0.0, 4.0, 5.0, 6.0, 7.0, 8.0, 9.0, 0.0, 0.0]
    assert len(_blocks._STRIDED_INDICES_CACHE) == 0


def test_non_contiguous_gather_still_uses_cached_indices():
    _blocks._clear_strided_indices_cache_for_tests()
    data = jnp.arange(12.0)
...
def test_size_one_dimension_with_arbitrary_stride_uses_contiguous_fast_path():
    ...
    gathered = _blocks.gather_strided(data, (2, 1, 3), (3, 999, 1), 10)
    scattered = _blocks.scatter_add_strided(
    ...
    assert gathered.tolist() == [[[10.0, 11.0, 12.0]], [[13.0, 14.0, 15.0]]]
    assert scattered.tolist() == [
...
def test_jitted_contiguous_gather_with_dynamic_offset_uses_uncached_indices():
    _blocks._clear_strided_indices_cache_for_tests()
    data = jnp.arange(12.0)

    @jax.jit
    def gather_with_offset(offset):
        return _blocks.gather_strided(data, (2, 3), (3, 1), offset)

    result = gather_with_offset(jnp.asarray(4))

    assert result.tolist() == [[4.0, 5.0, 6.0], [7.0, 8.0, 9.0]]
    assert len(_blocks._STRIDED_INDICES_CACHE) == 0


def test_jitted_contiguous_scatter_with_dynamic_offset_uses_uncached_indices():
    _blocks._clear_strided_indices_cache_for_tests()
    storage = jnp.zeros((12,), dtype=jnp.float32)
    value = jnp.array([[4.0, 5.0, 6.0], [7.0, 8.0, 9.0]], dtype=jnp.float32)

    @jax.jit
    def scatter_with_offset(offset):
        return _blocks.scatter_add_strided(storage, (2, 3), (3, 1), offset, value)

    result = scatter_with_offset(jnp.asarray(4))
